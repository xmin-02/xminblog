 ...
 *
 * Admin review routes:
 *   GET    /api/admin/review-posts                 — list posts waiting for review
 *   GET    /api/admin/review-posts/:slug           — get review post with content
 *   PUT    /api/admin/review-posts/:slug           — edit review post + metadata
...
function buildMarkdown(meta: PostMeta, body: string): string {
  ...
  const privateFields = meta.is_private
    ...
    : '';
  const reviewFields = [
    meta.source_type ? `source_type: "${meta.source_type.replace(/"/g, '\\"')}"` : '',
    meta.source_url ? `source_url: "${meta.source_url.replace(/"/g, '\\"')}"` : '',
    meta.source_id ? `source_id: "${meta.source_id.replace(/"/g, '\\"')}"` : '',
    meta.auto_generated ? 'auto_generated: true' : '',
    meta.review_status ? `review_status: "${meta.review_status}"` : '',
    meta.reviewed_at ? `reviewed_at: ${meta.reviewed_at}` : '',
...
function parseFrontmatter(raw: string): { meta: Partial<PostMeta>; body: string } {
  ...
  meta.auto_generated = bool('auto_generated') ?? false;
  meta.review_status = normalizeReviewStatus(str('review_status'), undefined);
  const reviewedAt = Number(str('reviewed_at'));
  meta.reviewed_at = Number.isFinite(reviewedAt) && reviewedAt > 0 ? reviewedAt : null;
  return { meta, body };
}
...
function normalizeReviewStatus(value: unknown, fallback: ReviewStatus | undefined = 'published'): ReviewStatus | undefined {
  const status = String(value ?? '').trim().toLowerCase() as ReviewStatus;
  if (REVIEW_STATUSES.has(status)) return status;
  return fallback;
}

function cleanShortText(value: unknown, fallback = '', max = 120): string {
...
function cleanSourceUrl(value: unknown): string {
  ...
  if (!url) return '';
  if (url.length > 1000) throw new Error('source_url is too long');
  if (!/^https?:\/\/[^\s]+$/i.test(url)) throw new Error('source_url must be an HTTP(S) URL');
  return url;
}
...
function reviewMetaFromRow(row: any) {
  return {
    ...
    auto_generated: dbBool(row.auto_generated),
    review_status: normalizeReviewStatus(row.review_status, 'published'),
    reviewed_at: row.reviewed_at ?? null,
    updated_at: row.updated_at ?? null,
  };
}

function reviewMetaFromPayload(payload: Partial<PostPayload>, existing?: any) {
  const draft = payload.draft ?? (existing ? dbBool(existing.draft) : false);
  const autoGenerated = payload.auto_generated ?? (existing ? dbBool(existing.auto_generated) : false);
  const defaultStatus: ReviewStatus = draft ? 'pending' : 'published';
  return {
    source_type: cleanShortText(payload.source_type ?? existing?.source_type ?? (autoGenerated ? 'automation' : 'manual'), 'manual', 80) || 'manual',
    source_url: cleanSourceUrl(payload.source_url ?? existing?.source_url ?? ''),
    source_id: cleanShortText(payload.source_id ?? existing?.source_id ?? '', '', 300),
    auto_generated: !!autoGenerated,
    review_status: normalizeReviewStatus(payload.review_status ?? existing?.review_status, defaultStatus) ?? defaultStatus,
    reviewed_at: payload.reviewed_at ?? existing?.reviewed_at ?? null,
  };
}

function dbPostSummary(row: any, includeReview = false) {
  const summary = {
...
async function getPost(slug: string, request: Request, env: Env, origin: string | null): Promise<Response> {
  ...
  if (contentBackend(env) === 'db') {
    ...
    const summary = dbPostSummary(row, isAdmin);
...
async function createPost(payload: PostPayload, env: Env, origin: string | null, ctx?: ExecutionContext): Promise<Response> {
  ...
  if (contentBackend(env) === 'db') {
    ...
    let reviewMeta: ReturnType<typeof reviewMetaFromPayload>;
    try {
      reviewMeta = reviewMetaFromPayload(payload);
    } catch (err) {
      return json({ error: (err as Error).message }, 400, origin);
    }
    ...
    `).bind(
      ...
      reviewMeta.source_url,
      reviewMeta.source_id,
      reviewMeta.auto_generated,
      reviewMeta.review_status,
      reviewMeta.reviewed_at,
      payload.content,
...
async function updatePost(slug: string, payload: PostPayload, env: Env, origin: string | null): Promise<Response> {
  ...
  if (contentBackend(env) === 'db') {
    ...
    let reviewMeta: ReturnType<typeof reviewMetaFromPayload>;
    try {
      reviewMeta = reviewMetaFromPayload(payload, existing);
    } catch (err) {
      return json({ error: (err as Error).message }, 400, origin);
    }
...
function reviewBackendRequired(env: Env, origin: string | null): Response | null {
  return contentBackend(env) === 'db'
    ? null
    : json({ error: 'Review workflow requires the db content backend' }, 501, origin);
}

async function listReviewPosts(request: Request, env: Env, origin: string | null): Promise<Response> {
  const admin = await requireAdminRequest(request, env, origin);
  if (admin instanceof Response) return admin;
  ...
  const status = statusParam === 'all' ? 'all' : normalizeReviewStatus(statusParam, 'pending');
  ...
  const result = await env.DB.prepare(`${baseSelect}${where} ORDER BY updated_at DESC, date DESC LIMIT ?`)
    .bind(...params, limit)
    .all<any>();

  return json((result.results ?? []).map(row => dbPostSummary(row, true)), 200, origin);
}

async function getReviewPost(slug: string, request: Request, env: Env, origin: string | null): Promise<Response> {
  const admin = await requireAdminRequest(request, env, origin);
  if (admin instanceof Response) return admin;
  const backendError = reviewBackendRequired(env, origin);
  if (backendError) return backendError;
  ...
  const row = await readPostRow(slug, env);
  if (!row) return json({ error: 'Post not found' }, 404, origin);
  return json({ ...dbPostSummary(row, true), content: row.content ?? '' }, 200, origin);
}

async function updateReviewPost(slug: string, request: Request, env: Env, origin: string | null): Promise<Response> {
  const admin = await requireAdminRequest(request, env, origin);
  if (admin instanceof Response) return admin;
  const backendError = reviewBackendRequired(env, origin);
  if (backendError) return backendError;
  ...
  try { body = await request.json() as Partial<PostPayload>; } catch { return json({ error: 'Invalid JSON' }, 400, origin); }

  const next: PostPayload = {
    password: '',
    ...
    auto_generated: body.auto_generated ?? dbBool(existing.auto_generated),
    review_status: body.review_status ?? normalizeReviewStatus(existing.review_status, 'pending'),
    reviewed_at: body.reviewed_at ?? existing.reviewed_at ?? null,
  };

  if (!next.title || !next.date || !next.category || !next.content) {
...
async function setReviewPostStatus(slug: string, action: 'approve' | 'publish' | 'reject', request: Request, env: Env, origin: string | null, ctx?: ExecutionContext): Promise<Response> {
  ...
  const now = Math.floor(Date.now() / 1000);
  const statusByAction: Record<typeof action, ReviewStatus> = {
    approve: 'approved',
    publish: 'published',
    reject: 'rejected',
  };
  ...
  const isPrivate = action === 'publish' ? false : dbBool(existing.is_private);

  await env.DB.prepare(`
    UPDATE posts
    SET draft = ?, is_private = ?, review_status = ?, reviewed_at = ?, updated_at = ?
    WHERE slug = ?
  `).bind(draft, isPrivate, status, now, now, slug).run();
...
export default {
  async fetch(request: Request, env: Env, ctx?: ExecutionContext): Promise<Response> {
    ...
    if (reviewMatch) {
      ...
      const action = reviewMatch[2] as 'approve' | 'publish' | 'reject' | undefined;
